...
export default async function CursoProfesorDireccionPage({ params }: { params: Promise<{ id: string; cursoId: string }> }) {
  ...
  const sesionesMap = new Map(sesiones?.map(s => [s.fecha, s.id]) || [])
  ...
  if (modulo.fecha_inicio && modulo.fecha_fin) {
    ...
    while (cur <= finMes) {
      ...
      for (let d = 1; d <= diasEnMes; d++) {
        ...
        else if (sesionesMap.has(fecha)) { tipo = 'clase'; sesionId = sesionesMap.get(fecha) }
  ...
  return (
    <div>
          ...
          <div key={mes.label} style={{ border:'0.5px solid #E8DFCF', borderRadius:'8px', overflow:'hidden' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(7, 1fr)', gap:'1px', padding:'4px', background:'#f5f5f5' }}>
              ...
              {mes.diasGrid.map((d, i) => {
                ...
                return (
                  <div key={i} style={{ fontSize:'9px', textAlign:'center', padding:'2px 1px', background:bg, color:col, borderRadius:'2px', fontWeight: d.tipo !== 'normal' ? 700 : 400 }}>
                    {d.dia}
                  </div>
                )
              })}
      ...
      <div style={{ display:'flex', gap:'12px', marginBottom:'20px', fontSize:'9px', color:'#666', flexWrap:'wrap' }}>
